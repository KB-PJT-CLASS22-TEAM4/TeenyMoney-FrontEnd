    ...
    <div class="scroll">
      ...
      <div class="content">
        ...
        <div class="field">
          ...
          <p v-if="errors.birthDate" class="error-text">
            {{ errors.birthDate }}
          </p>
        </div>
        ...
        <div class="field">
          ...
          <div class="input-row">
            ...
            <span v-if="timerActive" class="timer">
              {{ formattedTimer }}
            </span>
          </div>
        ...
        >
          <div class="modal">
            ...
            <!-- 보호자 이름 -->
            <div class="field">
              <label class="label" for="guardianName"> 보호자명 </label>
            ...
            <!-- 보호자 관계 -->
            <div class="field">
              <label class="label" for="guardianRelationship"> 관계 </label>
            ...
            <!-- 보호자 휴대전화 -->
            <div class="field">
              <label class="label" for="guardianPhone"> 휴대전화 </label>
              ...
              <p v-if="guardianCodeSent" class="sent-text">
                인증번호가 발송되었습니다
              </p>
            </div>
            ...
            <!-- 보호자 인증번호 -->
            <div class="field">
              <label class="label" for="guardianCode"> 인증번호 </label>
...
function validateBirthDate() {
  ...
  if (digits.length !== 8) {
    errors.birthDate = '생년월일을 완전히 입력해 주세요.';
    return false;
  }
  ...
  if (isInvalidDate) {
    errors.birthDate = '올바른 날짜를 입력해 주세요.';
    return false;
  }
...
function formatPhoneNumberInput(event) {
  form.phoneNumber = applyPhoneFormat(event.target.value);
}
...
function formatGuardianPhoneInput(event) {
  guardian.phone = applyPhoneFormat(event.target.value);
...
function formatGuardianCodeInput(event) {
  guardian.code = event.target.value.replace(/\D/g, '').slice(0, 6);
}
...
function validatePassword() {
  ...
  if (form.email && password.includes(form.email)) {
...
const submitBlockReason = computed(() => {
  ...
  if (!form.birthDate.trim() || form.birthDate.length !== 10) {
  ...
  if (formatPhone(form.phoneNumber).length !== 11) {
  ...
  if (form.password.length < 10) {
    return `비밀번호가 ${form.password.length}자예요. ` + '(10자 이상 필요)';
  }
...
async function requestVerification() {
  ...
  try {
    ...
    if (!response.success) {
      alert(response.message || '인증번호 발송에 실패했어요.');
      return;
    }
    ...
    console.error('본인 인증번호 발송 실패:', error);
...
async function requestGuardianVerification() {
  const legalGuardianName = guardian.name.trim();

  const guardianPhone = formatPhone(guardian.phone);
  ...
  if (guardianPhone.length !== 11) {
    alert('보호자 휴대전화 번호를 정확히 입력해 주세요.');
    return;
  }

  try {
    const response = await sendGuardianVerificationCode(guardianPhone);

    if (!response.success) {
      alert(response.message || '보호자 인증번호 발송에 실패했어요.');
      return;
    }
    ...
    console.error('보호자 인증번호 발송 실패:', error);

    alert('보호자 인증번호 발송에 실패했어요.');
  }
}
...
async function verifyGuardianCode() {
  ...
  if (!guardianCodeSent.value) {
    alert('먼저 보호자 인증번호를 발송해 주세요.');
    return;
  }

  if (guardian.code.length !== 6) {
    alert('보호자 인증번호 6자리를 입력해 주세요.');
    return;
  }
  ...
  try {
    ...
    console.log('보호자 인증 확인 요청:', requestData);

    const response = await confirmGuardianVerification(requestData);

    console.log('보호자 인증 확인 응답:', response);

    if (!response.success) {
      alert(response.message || '보호자 인증번호가 올바르지 않아요.');
      return;
    }
    ...
    alert('보호자 인증이 완료됐어요! 가입 완료 버튼을 눌러 주세요.');
  } catch (error) {
    console.error('보호자 인증 확인 실패:', error);

    alert('보호자 인증번호 확인에 실패했어요.');
  }
}
...
function toggleTerms() {
  ...
  if (isUnder14() && !guardianVerified.value) {
...
async function submit() {
  if (!canSubmit.value || signupLoading.value) {
    return;
  }
...
async function doSignup() {
  ...
  try {
    ...
    console.log('회원가입 요청 JSON:', JSON.stringify(requestData));
    ...
    if (!response.success) {
      alert(response.message || '회원가입에 실패했어요.');
      return;
    }
